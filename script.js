...
function generateSongsList() {
  ...
  for (let i = 0; i < G.songsList.length; i++) addsongtolist(i);
}
...
function updateImage(i) {
  if (i == -1)
    G.BGnumber = Math.floor(Math.floor(Math.random() * G.BGimages.length));
  // comment out this line if you do not want random images from the list
  else if (G.BGnumber + i >= G.BGimages.length) G.BGnumber = 0;
  else if (G.BGnumber + i < 0) G.BGnumber = G.BGimages.length - 1;
  else G.BGnumber += i;
  if (G.BGnumber)
    if (G.BGimages[G.BGnumber] == undefined)
      alert("The wallpaper according to the number do not exist.");

  let css = `background-image: url(\"${G.BGimages[G.BGnumber]}\") ; height:${
    window.innerHeight
